...
def p_lista_variaveis(p):
    ...
    if len(p) > 2:
        ...
        # p[2] = filho #inseri, mas n tinha, sera que precisa????????
        p[3].parent = pai
...
def p_indice(p):
    ...
    i = 0
    if len(p) == 5:
        p[i+1].parent = pai   # indice
        i += 1

    filho_abre = MyNode(name='abre_colchete', type='ABRE_COLCHETE', parent=pai)
    filho_abre_sym = MyNode(name=p[i+1], type='SIMBOLO', parent=filho_abre)
    p[i+1] = filho_abre

    p[i+2].parent = pai  # expressao

    filho_fecha = MyNode(name='fecha_colchete', type='FECHA_COLCHETE', parent=pai)
    filho_fecha_sym3 = MyNode(name=p[i+3], type='SIMBOLO', parent=filho_fecha)
    p[i+3] = filho_fecha
...
#     print("Erro:p[0]:{p0}, p[1]:{p1}, p[2]:{p2}, p[3]:{p3}, p[4]:{p4}, p[5]:{p5}, p[6]:{p6}".format(
...
def p_parametro_error(p):
    ...
    """
    print("Erro na definicao do parametro.")
    error_line = 0 
...
def p_se_error(p):
  ...
  """
    print("Erro na definicao da acao SE.")
...
def p_repita_error(p):
    ...
    """
    print("Erro na definicao da acao REPITA.")
...
def p_leia_error(p):
    ...
    """
    print("Erro na definicao da acao LEIA.")
...
def p_chamada_funcao(p):
    ...
    if len(p) > 2:
        ...
        p[1].parent = pai #nao entendi esse caso
...
def p_error(p):
    if p:
        token = p
        print("Erro próximo ao token '{token}'".format(token=token.value))
        # parser.errok()
...
def main():
    ...
    data = open(argv[1], encoding="utf-8")
